...
import com.dawn.nav.authentication.handler.login.github.GitHubAuthenticationFilter;
import com.dawn.nav.authentication.handler.login.github.GitHubAuthenticationProvider;
import com.dawn.nav.authentication.handler.login.linuxDo.LinuxDoAuthenticationFilter;
import com.dawn.nav.authentication.handler.login.linuxDo.LinuxDoAuthenticationProvider;
...
public class SecurityConfig {
  ...
  /**
   * 用于加密用户密码:提供一个BCryptPasswordEncoder实例，用于加密用户密码。
   *
  ...
  /**
   * 公共配置：应用通用的安全配置，包括禁用默认的表单登录、HTTP基本认证、登出、会话管理和CSRF防护。配置自定义的异常处理，并添加全局异常处理过滤器
   */
...
// 应用程序能够灵活地处理不同类型的认证请求，并且确保不同的API路径具备相应的安全措施，同时提供统一的异常处理机制